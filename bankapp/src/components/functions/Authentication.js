    ...
    return new Promise((resolve, reject) => {
        axios
            ...
            .then((response) => {
                ...
                saveClientInfo(response.data.loginAuth);
                resolve(response)
            })
            .catch((err) => {
...
const loginUser = (response) => {
    return new Promise((resolve, reject) => {
        if (response.data.confirm === "d688c4ccd7e20183b67f80d8816a2126") {
...
const saveClientInfo = (response) => {
    globalStore.clientData = response;
}
